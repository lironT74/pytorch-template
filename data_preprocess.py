...
def questions_data_words_to_indexes(train_question, name, cache_root):
    ...
    cache_file = os.path.join(cache_root, name+'_words2index.pkl')
    pickle.dump(word2index, open(cache_file, 'wb'))

    cache_file = os.path.join(cache_root, name+'_index2words.pkl')
    pickle.dump(index2word, open(cache_file, 'wb'))



def v2_questions_words_dicts():

    train_question_file = '/datashare/v2_OpenEnded_mscoco_train2014_questions.json'
